    ...
    <div className="min-h-screen bg-[#F8FAFC]">
      ...
      <main className="mx-auto max-w-7xl px-6 py-12">
        <div className="border-b border-[#CCCCCC] pb-6">
          <h1 className="text-3xl font-extrabold tracking-tight text-[#0B2545]">Academic Innovation Panel</h1>
          <p className="mt-1 text-xs font-semibold text-slate-600 uppercase tracking-wider">
            Assigned Student Group: <strong className="text-[#E65C00] font-bold">{teamName}</strong>
          </p>
        </div>
        ...
        {loading ? (
          ...
          <div className="mt-8 grid gap-8 lg:grid-cols-3">
            ...
            <div className="lg:col-span-1 bg-white border border-[#CCCCCC] rounded-[2px] p-6 shadow-none">
              <h2 className="text-base font-bold text-[#0B2545] border-b border-[#CCCCCC] pb-3 uppercase tracking-wider">
                ...
                {problems.length === 0 ? (
                  ...
                  problems.map((problem) => {
                    ...
                    return (
                      <div
                        key={problem._id}
                        className="rounded-[2px] border border-[#CCCCCC] bg-slate-50/50 p-4 hover:border-[#E65C00] transition-colors"
                      >
                        <div className="flex justify-between items-center text-[10px] font-bold text-slate-500">
                          <span>📍 {problem.location}</span>
                          <span className="text-emerald-700 bg-emerald-50 px-2 py-0.5 rounded-[2px] border border-emerald-100 uppercase">
                            ★ {score}% MATCH
                          </span>
                        </div>
                        <h4 className="font-bold text-[#0B2545] mt-2 text-xs uppercase">{problem.title}</h4>
                        ...
                        <div className="mt-4 flex items-center justify-between text-[10px] font-bold">
                          <span className="bg-slate-100 border border-[#CCCCCC] rounded-[2px] px-2 py-0.5 text-slate-700 uppercase">
                            {problem.category}
                          </span>
                          <button
                            onClick={() => handleAcceptProblem(problem)}
                            disabled={actionLoading}
                            className="rounded-[2px] bg-[#E65C00] px-2.5 py-1 text-[10px] text-white hover:bg-[#C24E00] font-bold disabled:opacity-50"
                          >
                            ACCEPT PROJECT
                          </button>
                        </div>
                ...
                <div className="mt-6 space-y-6">
                  {projects.filter(p => p.teamName === teamName).length === 0 ? (
                    ...
                    projects
                      ...
                      .map((project) => (
                        <div key={project._id} className="rounded-[2px] border border-[#CCCCCC] p-5 bg-white shadow-none border-l-4 border-l-[#0B2545]">
                          <div className="flex justify-between items-start gap-4 flex-wrap">
                            <div>
                              <h3 className="font-bold text-[#0B2545] text-sm uppercase">{project.title}</h3>
                              <p className="text-[10px] text-slate-500 mt-1 font-semibold">
                                STATUS: <span className="text-indigo-700 uppercase">{project.status.replace("_", " ")}</span>
                              </p>
                            </div>
                            <span className="text-[10px] bg-slate-100 border border-[#CCCCCC] px-2.5 py-1 rounded-[2px] font-bold text-slate-700 uppercase">
                          ...
                          <div className="mt-6">
                            ...
                            <div className="h-2 w-full rounded-[2px] bg-slate-100 overflow-hidden border border-[#CCCCCC]">
                              <div
                                style={{ width: `${project.progress}%` }}
                                className="h-full bg-[#0B2545] transition-colors duration-100"
                              ></div>
                            </div>
                          </div>

                          {/* Industry support status */}
                          <div className="mt-4 bg-[#FFFBEB] rounded-[2px] p-3 border border-amber-200 text-xs leading-relaxed">
                            {project.industryPartner ? (
                              <p className="text-amber-800">
                                🤝 Supported by <strong className="font-bold text-[#0B2545]">{project.industryPartner}</strong>: Offers {project.industrySupport.join(", ")}.
                              </p>
                            ) : (
                          ...
                          <div className="mt-4 flex justify-end gap-3">
                            <button
                              ...
                              className="rounded-[2px] bg-[#0B2545] px-4 py-1.5 text-[10px] font-bold text-white hover:bg-slate-800 disabled:opacity-50 uppercase tracking-wider"
                            >

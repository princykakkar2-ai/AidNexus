    ...
    <div className="min-h-screen bg-[#F8FAFC]">
      ...
      <main className="mx-auto max-w-7xl px-6 py-12">
        <div className="border-b border-slate-200 pb-6">
          <h1 className="text-3xl font-extrabold tracking-tight text-[#0F172A]">Industry & NGO Sponsorship Hub</h1>
        ...
        {loading ? (
          ...
          <div className="mt-12 text-center text-slate-500 py-12 bg-white rounded-[6px] border border-slate-200 shadow-sm">
            ...
            {projects.map((project) => (
              ...
              >
                <div className="flex items-center justify-between">
                  <span className="badge-gov bg-slate-50 border border-slate-200 text-slate-700">
                    {project.status.replace("_", " ")}
                  </span>
                  <span className="text-[10px] font-bold text-slate-500">Progress: {project.progress}% Complete</span>
                </div>

                <h3 className="mt-4 text-base font-bold text-[#0F172A] uppercase">{project.title}</h3>
                <p className="mt-1 text-xs text-slate-600">
                  Assigned Team: <strong className="font-bold text-slate-800 uppercase">{project.teamName}</strong>
                </p>

                <div className="mt-4 bg-slate-50 rounded-[4px] p-4 border border-slate-200 flex-1">
                  ...
                  <div className="mt-2 flex flex-wrap gap-2">
                    {project.supportNeeded.length === 0 ? (
                      ...
                      project.supportNeeded.map((item) => (
                        <span key={item} className="rounded-[4px] bg-amber-50 border border-amber-200 text-amber-800 px-2 py-0.5 text-[10px] font-bold uppercase">
                          {item}
                        </span>
                      ))
                    )}
                ...
                {project.industryPartner ? (
                  <div className="mt-6 bg-emerald-50 border border-emerald-100 rounded-[4px] p-4 text-xs text-emerald-800 font-medium">
                    🤝 SPONSORED BY <strong className="font-bold text-[#0F172A]">{project.industryPartner}</strong>: Provided {project.industrySupport.join(", ")}.
                  </div>
                ) : (
                  <div className="mt-6">
                    <button
                      onClick={() => handleOpenSupportModal(project)}
                      className="w-full rounded-[4px] bg-[#D97706] px-5 py-2.5 text-xs font-bold text-white hover:bg-[#B45309] shadow-sm transition-all uppercase tracking-wider"
                    >
        ...
        {activeProject && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 p-4 backdrop-blur-sm">
            <div className="w-full max-w-md rounded-[6px] bg-white p-6 shadow-xl border border-slate-200">
              <h3 className="text-lg font-bold text-[#0F172A] uppercase tracking-wide">Register Support Offer</h3>
              ...
              <form onSubmit={handleSubmitSupport} className="mt-6 space-y-4">
                ...
                {error && (
                  <p className="text-xs text-red-600 bg-red-50 p-2.5 rounded-[4px] border border-red-200">{error}</p>
                )}
                ...
                <div className="mt-6 flex justify-end gap-3">
                  <button
                    ...
                    className="rounded-[4px] border border-slate-300 bg-white px-4 py-2 text-xs font-bold text-slate-700 hover:bg-slate-50"
                  >
                    CANCEL
                  </button>
                  <button
                    type="submit"
                    className="rounded-[4px] bg-[#D97706] px-5 py-2 text-xs font-bold text-white hover:bg-[#B45309]"
                  >
                    SUBMIT SPONSORSHIP
                  </button>
                </div>
